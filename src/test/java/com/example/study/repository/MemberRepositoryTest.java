 ...
 */
    ...
    void testQueryDSL() {
        // given
        // QueryDSL에서 사용할 q클래스 객체를 받아온다. (클래스 내에 상수로 객체가 전언되어 있기 때문에에 직접 생성할 필요 없음)
        QMember m = member; // jps가 이렇게 바뀐거라고 생각하면 된다.
        //when
    ...
    void search() {
        ...
        //when
        Member foundMember = factory.selectFrom(member) // * 과 동일하다. SELECT * FROM
...
//                    .where(원하는 조건)
    ...
    void paging() {
        ...
        // then
        assertEquals(result.size(), 3); // 단원기법
    ...
    void aggregation() {
        ...
        List<Tuple> result = factory.select(
                ...
                .from(member)
                .fetch(); // 리스트로 반환 하기 위해 사용

        // Tuple: QueryDSL에서 쿼리로 나온 결과 행등을 타입에 맞춰 담을 수 있게 제공되는 타입.
    ...
    void subQueryTest() {
        ...
        List<Member> result = factory.selectFrom(member)
                .where(member.age.eq(
                        // 나이가 가장 많은 사람을 조회하는 서브퀴리문이 들어가야 함.
                        JPAExpressions // 서브퀴리를 사용할 수 있게 해주는 클래스
